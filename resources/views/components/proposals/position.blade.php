...
<div @class([
    ...
    @if ($position < 3)
        <x-ui.icons.trophy class="h-[24px] w-[24px]" />
    @else
        @if ($status == 'up')
            <x-ui.icons.arrow-up class="h-[24px] w-[24px]" />
        @elseif($status == 'down')
            <x-ui.icons.arrow-down class="h-[24px] w-[24px]" />
        @else
            <x-ui.icons.minus class="h-[24px] w-[24px]" />
        @endif
    @endif
</div>
